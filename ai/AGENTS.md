...
Default guidance for AI coding agents (Codex, Claude Code, Copilot) working on **any** of Izaias's repositories.

This file lives at `ai/AGENTS.md` in the [dotfiles](https://github.com/izaiasmachado/dotfiles) repo. It is installed as the global agent config via `make agents` from the dotfiles root:

- `~/.codex/AGENTS.md` → this file (Codex CLI loads it on every session)
- `~/.claude/CLAUDE.md` → this file (Claude Code loads it on every session)

The rules below apply to **every repo** Codex/Claude touch. A repo can override or extend them with its own root `AGENTS.md` / `CLAUDE.md` — including this dotfiles repo itself, which carries a project-specific [AGENTS.md](../AGENTS.md) at its root for scopes, Makefile pointers, and other dotfiles-only conventions.
